...
def vul_lst(v1, v2, v3):
	...
	"""
	y = ""
	x=[vul_class(v2)+"%$ "+str(tool_resp[v1][0])+"%$ "+str(tools_fix[v3-1][1])+"%$ "+str(tools_fix[v3-1][2])]

	if vul_class(v2) == "CRITICAL":
		y = "CRITICAL"
	elif vul_class(v2) == "HIGH":
		y = "HIGH"
	elif vul_class(v2) == "MEDIUM":
		y = "MEDIUM"
	elif vul_class(v2) == "LOW":
		y = "LOW"
	elif vul_class(v2) == "INFO":
		y = "INFO"
	else:
		y = "na"
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
		...
		rs_skipped_checks = rs_skipped_checks + 1
            ...
            if runTest == 1:
				...
				testone=vul_lst(tool,tool_resp[tool][arg2],tool_resp[tool][arg3])[0]

				res = vul_lst(tool,tool_resp[tool][arg2],tool_resp[tool][arg3])[1]
				if res == "CRITICAL":
					crit.append(testone)
				elif res == "HIGH":
					high.append(testone)
	...
	print(len(resultsdict))
		...
		with open("threatreport.txt", "w")as r:
			for i in crit:				
				r.write(str(i)+"\n")
			for i in high:
				r.write(str(i)+"\n")
